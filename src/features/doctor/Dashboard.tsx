...
const DoctorDashboard: React.FC = () => {
  ...
  if (!authUser) {
    return (
      <div className="flex">
        <Sidebar />
        <div className="ml-64 p-6 text-red-600">
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-gray-100">
      {/* Floating shapes */}
      ...
      <main className="ml-64 flex flex-col items-center justify-start pt-24 space-y-10 px-6 md:px-16">
        ...
        {doctorId ? (
          ...
          <div className="text-red-600 mb-6 text-center">Doctor ID missing in your profile.</div>
        )}

        {/* Appointments Card */}
        <section className="w-full max-w-5xl bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl p-8">
            ...
            <div className="grid gap-6">
              {appointments.map((appt) => {
                ...
                return (
                  <div
                    key={appt._id}
                    className="flex justify-between items-center bg-white rounded-xl shadow-md p-4 hover:shadow-xl transition"
                  >
                    <div className="flex flex-col">
                      <span className="font-semibold text-lg">{patient.name}</span>
                      <span className="text-gray-500">
                        {new Date(appt.date).toLocaleDateString("en-GB")} at {appt.time}
                      </span>
                    </div>
                    <div
