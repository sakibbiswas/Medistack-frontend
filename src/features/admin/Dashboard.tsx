...
const AdminDashboard: React.FC = () => {
  ...
  const summaryCards = [
    { label: "Users", value: users?.length ?? 0, icon: <FaUsers />, gradient: "from-blue-500 to-blue-400" },
    { label: "Doctors", value: doctors?.length ?? 0, icon: <FaUserMd />, gradient: "from-green-500 to-green-400" },
    { label: "Appointments", value: appointments?.length ?? 0, icon: <FaCalendarAlt />, gradient: "from-purple-500 to-purple-400" },
    { label: "Payments", value: payments?.length ?? 0, icon: <FaCreditCard />, gradient: "from-yellow-400 to-yellow-300" },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="ml-64 flex-1 p-4 md:p-14">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {summaryCards.map((card) => (
                  ...
                  >
                    ...
                    <div>
                      <div className="text-sm md:text-base font-medium opacity-80">{card.label}</div>
